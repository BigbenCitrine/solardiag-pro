  ...
  try {
    ...
    if (isFree) {
      ...
      if (currentWeekUsage >= weeklyLimit) {
        return NextResponse.json({
          error: `Ai atins limita săptămânală de ${weeklyLimit} analize pentru contul FREE. Upgradează la PRO pentru acces nelimitat.`
        }, { status: 429 });
      }
    }
    ...
    if (isProOrAbove && openai) {
      try {
        ...
        finalResult = `🤖 **ANALIZĂ AI (PRO/INSTALLER):**\n\n${aiResponse}`;
